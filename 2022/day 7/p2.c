...
int totalSize = 0;
int smallestDirSize = 70000000; // basically infinity
...
int search(File* parentNode) {
    ...
    // that number is the total size...but I ran the program, looked at what that number was, then put that number back into the program. Sorry :P.
    if (dirSize > 4125990 && dirSize < smallestDirSize)
...
int main() {
    ...
    totalSize = search(&root);
    printf("Total size = %d\n", totalSize);

    printf("Smallest dir size = %d\n", smallestDirSize);
